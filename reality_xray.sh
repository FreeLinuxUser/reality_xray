...
getkey() {
	...
	private_key=$(grep -i "Private" /usr/local/etc/xray/key | sed 's/.*:[[:space:]]*//')
    # 兼容新老版公钥：老版叫 Public key，新版叫 Password
    public_key=$(grep -E -i "Public|Password" /usr/local/etc/xray/key | sed 's/.*:[[:space:]]*//')
